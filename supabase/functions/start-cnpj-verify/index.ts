// O ROLÊ+ — start-cnpj-verify (Supabase Edge Function)
// Passo 1 da verificação de posse no CADASTRO DIRETO de negócio (o "Com CNPJ,
// sem catálogo prévio" — diferente da reivindicação de empresa já catalogada,
// que usa start-claim). Recebe um CNPJ, confere que ele ainda não tem conta
// registrada, busca o e-mail oficial na Receita Federal e manda o código de
// confirmação pra lá (nunca pro e-mail que a pessoa digitar no formulário).
// Não exige login — a conta só é criada depois que o código é confirmado.
...
function renderEmailHtml(codigo: string, cnpjFmt: string, razaoSocial: string) {
  // mesmo estilo do template de supabase/email-templates/confirmar-cnpj.html
  ...
  <p style="margin:0 0 4px;font-size:19px;font-weight:700;color:#211417;">Confirme que o CNPJ é seu</p>
  <p style="margin:0 0 28px;font-size:14.5px;line-height:1.6;color:#6B565B;">Alguém está criando uma conta de negócio no Rolê+ usando o CNPJ <b style="color:#211417;">${cnpjFmt}</b> (<b style="color:#211417;">${razaoSocial}</b>). Digite o código abaixo no app pra confirmar que é você.</p>
  </td></tr>
  ...
  <p style="margin:0;font-size:13px;line-height:1.6;color:#8C7A7E;">Esse código vale por ${CODE_TTL_MIN} minutos. Se você não pediu esse cadastro, pode ignorar este e-mail — nenhuma conta é criada até alguém confirmar o código.</p>
  </td></tr>
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // 1) já existe conta registrada com esse CNPJ? não deixa duplicar/roubar
    const { data: existing, error: existingErr } = await supabase
      .from('profiles').select('id').eq('cnpj', digits).maybeSingle();
    if (existingErr) return new Response(JSON.stringify({ error: existingErr.message }), { status: 500, headers: CORS });
    if (existing) {
      return new Response(JSON.stringify({ error: 'já existe uma conta cadastrada com esse CNPJ' }), { status: 409, headers: CORS });
    }

    // 2) busca o e-mail oficial na Receita Federal
    const cnpjRes = await fetch('https://brasilapi.com.br/api/cnpj/v1/' + digits);
    if (!cnpjRes.ok) {
      return new Response(JSON.stringify({ error: 'CNPJ não encontrado na Receita Federal' }), { status: 404, headers: CORS });
    }
    const cnpjData = await cnpjRes.json();
    const officialEmail: string | undefined = cnpjData.email || undefined;
    if (!officialEmail) {
      return new Response(JSON.stringify({ needsManualReview: true, reason: 'no_official_email' }), { headers: CORS });
    }
    ...
    const { data: inserted, error: insertErr } = await supabase.from('cnpj_signup_codes').insert({
      cnpj: digits, code: codigo, sent_to_masked: masked, expires_at: expiresAt,
    }).select('token').single();
    ...
    const razaoSocial = cnpjData.razao_social || cnpjData.nome_fantasia || '';
    ...
    const emailRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        html: renderEmailHtml(codigo, cnpjFmt, razaoSocial),
      }),
    });
    if (!emailRes.ok) {
    ...
    return new Response(JSON.stringify({ sentTo: masked, verifyToken: inserted.token }), { headers: CORS });
  } catch (e) {
